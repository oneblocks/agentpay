...
<p align="center">
  <img src="./logo.png" alt="AgentPay Logo" width="600">
</p>
...
<p align="center">
  <img src="./one-man-company.png" alt="One-Man Company" width="700">
</p>
...
<p align="center">
  <img src="./hire-agent.png" alt="Hire AI Agent" width="500">
</p>
...
- `/` : Router 核心逻辑，负责节点管理、路由分发与支付校验转发。
- `/agent` : 示例 Agent 节点代码，开发者可参考此目录快速搭建自己的收款节点。
- `/web` : 可视化管理面板，监控节点状态、查看实时流量与结算数据。

## 🏗 x402 支付协议架构

下集展示了基于 x402 协议的 AgentPay 核心支付流程：
...
### 运行 Router
1. 复制 `.env.example` 并重命名为 `.env`，配置 `PROVIDER_API_KEY` (用于语义路由)。
2. 运行项目：
   ```bash
   go run .
   ```
...
### 接入作为节点
请参考 [Agent 接入指南](./agent/README.md) 了解如何将你的 Agent 接入网络并开始收款。

---

## 💰 获取测试代币 (Faucet)

为了在 Monad 测试网上运行项目，你需要领取以下测试代币：

1.  **领取 MON (Gas 费)**：
    -   访问 [Monad Faucet](https://faucet.monad.xyz/)。
    -   作用：MON 是 Monad 链的本地代币，用于支付交易的手续费（Gas）。
2.  **领取 USDC (支付代币)**：
    -   访问 [Circle Faucet](https://faucet.circle.com/)。
    -   配置：选择 **Monad** 网络进行领取。
    -   作用：USDC 是 Agent 之间进行结算所使用的代币。
3.  **添加 Monad Testnet 网络**：
    -   访问 [ChainList](https://chainlist.org/)。
    -   配置：添加 **Monad Testnet** 网络。
